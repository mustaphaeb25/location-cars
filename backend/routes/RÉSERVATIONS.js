// 1. Route to create a new reservation - CORRECTED ENDPOINT
...
router.get('/user/:userId', (req, res) => {
  ...
  // CORRECTED SQL QUERY (removed comments and fixed column names)
  const query = `
    SELECT
      r.id AS id,
      r.id_utilisateur,
...
// 4. Update reservation status - FIXED
router.put('/:id/statut', (req, res) => { // The endpoint path is '/:id/statut'
  const reservationId = req.params.id;
  const { newStatus } = req.body; // <-- FIX: Destructure 'newStatus' from the body

  // --- Add validation for newStatus (Recommended) ---
